...
import {getClientTransferMarginList} from '../../../actions/client/summary/transactionAction';

import styles from '../../css/client/detail/profitList.css';

//交易 - 委托 - 两融银证转账
class TradeTransferMarginList extends CursorTable {
    ...
    getData(startIndex, isAppend, cb, props) {
        ...
        this.props.getClientTransferMarginList(params, isAppend, cb, this, this.updateList);
    }
        ...
        return data.map((item, index) => {
            ...
            return (
                <tr className={this.mergeClassName(styles.tr, styles.dataRow)} key={index}>
                    <td width="28%">
                        <p>{bankName}</p>
                        <p className={Color.c6}>{transDate}</p>
                    </td>
                    <td width="45%">
                    ...
                    <td width="27%">
                        <p className={Color.red}>{bktransStatus}</p>
                        <p className={Color.red}>{opType}</p>
                    </td>
                </tr>
            );
        });
...
function injectAction() {
    return {getClientTransferMarginList};
}

module.exports = connect(injectProps, injectAction(),null,{withRef:true})(TradeTransferMarginList);
