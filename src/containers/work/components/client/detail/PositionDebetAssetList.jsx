...
import {getClientDebetAssetList} from '../../../actions/client/summary/positionAction';

import styles from '../../css/client/detail/profitList.css';

//持仓 - 负债 - 融券
class PositionDebetAssetList extends CursorCachedTable {
    ...
    getCachedData(props, requestServer, cb){
        var data = Cache.getValue("client_position_debet_asset");
        if(data){
    ...
    updateList = (isAppend, data, hasMore) => {
        ...
        Cache.setValue("client_position_debet_asset",{list,hasMore});
    }

    getData(startIndex, isAppend, cb) {
        var {clientId} = this.props;
        this.props.getClientDebetAssetList({
            startIndex,
            clientId
...
function injectAction() {
    return {getClientDebetAssetList};
}

module.exports = connect(injectProps, injectAction(),null,{withRef:true})(PositionDebetAssetList);
